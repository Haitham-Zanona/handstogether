...
<form action="{{ route('admin.settings.update') }}" method="POST" class="space-y-8">
    ...
    <div class="bg-white rounded-lg shadow">
        <div class="px-6 py-4 border-b border-gray-200">
            <h3 class="flex items-center text-lg font-semibold text-gray-900">
                <svg class="w-5 h-5 ml-2 text-orange-500" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"/>
                </svg>
                إعدادات العام الدراسي
            </h3>
            <p class="mt-1 text-sm text-gray-500">يُستخدم تاريخ الانتهاء لحسبة رواتب العاملين في نهاية العام</p>
        </div>
        <div class="p-6">
            <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
                <div>
                    <label for="academic_year_end_date" class="block mb-2 text-sm font-medium text-gray-700">
                        تاريخ انتهاء العام الدراسي
                    </label>
                    <input type="date" id="academic_year_end_date" name="academic_year_end_date"
                        value="{{ old('academic_year_end_date', $academySettings['academic_year_end_date']) }}"
                        class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-orange-400 focus:border-orange-400">
                    @error('academic_year_end_date')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                    <p class="mt-1.5 text-xs text-gray-400">
                        إذا لم يكتمل الموظف/المدرس دورة الـ31 يوماً عند هذا التاريخ،
                        يُحسب راتبه بشكل تناسبي (الأيام × سعر اليوم).
                    </p>
                </div>
                @if(!empty($academySettings['academic_year_end_date']))
                <div class="flex items-start pt-6">
                    <div class="bg-orange-50 border border-orange-200 rounded-lg p-3 text-sm text-orange-800 w-full">
                        <p class="font-semibold mb-1">العام الدراسي الحالي ينتهي في:</p>
                        <p class="font-mono text-lg font-bold">{{ \Carbon\Carbon::parse($academySettings['academic_year_end_date'])->format('d / m / Y') }}</p>
                    </div>
                </div>
                @endif
            </div>
